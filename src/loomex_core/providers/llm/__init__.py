...
host 直接从本模块引入即可：
    ...
    from loomex_core.providers.llm import (
...
`AnthropicAdapter` / `OpenAIAdapter` 依赖 httpx（`pip install "loomex-core[llm]"`），因此采用
...
from loomex_core.providers.llm.provider import (
    SUPPORTED_STYLES,
    LLMAccount,
    LLMProvider,
    ModelConfig,
)
from loomex_core.providers.llm.store import LLMAccountStoreProtocol
