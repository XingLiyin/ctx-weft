...
class RunStateView:
    ...
    #: 1999 条折出 1 个，272ms / 5.1MB，读放大 1999×）。进了投影就随快照 + 增量走，
    #: 变成 O(delta)——与 `pending_hitl` 同一档待遇。
    #:
    #: **为什么它进得起投影**：每个条目由它自己的 `TASK_RECAP_DONE` 删掉，所以大小 =
    #: 「崩溃打断且尚未修复的 recap 数」≈ 0~2，与会话长度无关。
