"""事件封套的唯一构造点 + 唯一一份 EVENT_TYPES 白名单校验。

改造前「无 run 语境」的封套（`run_id=None, sequence=0`）在全仓有 9 处逐字重复，跨
`core.orchestrator`（7）/ `core.hitl`（1）/ `core.runtime`（1）三个包；白名单校验另有
3 份。收进这里。

**为什么和 clock / ids 同包**：`new_event` 就是用 `generate_id` + `now_utc` 造出来的。

**为什么不叫 `core/events.py`**：那个模块名被
`tests/unit/test_protocols_events_relocation.py` 明令禁止——旧的 `core/events/` 是
2026-08-27 三层划界时的转发 shim，08-29 删除，那条用例防的正是「有人为了少改一行
又把转发层加回来」。本模块不转发任何东西，但沿用那个名字会踩守卫、也会让人误以为
shim 复活。

run 级事件仍走 `core.loop.driver.make_event`——它做 LoopState 字段抽取 +
`sequence_counter` 自增，那是 run 域的真实职责；它只是委托本模块拼封套。
"""
