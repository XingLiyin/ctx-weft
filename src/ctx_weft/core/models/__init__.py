...
- **几乎都是叶子**——只引 `protocols`、`core.utils.content` 与彼此（唯一的例外见下）；
...
- `ContextOverflowError` 的报错文案在函数内引 `core.utils.content._IMAGE_PART_TOKENS`。
  这一处**必须**保持函数内 import：`core.utils.content` 模块级就引 `core.models.errors`，
