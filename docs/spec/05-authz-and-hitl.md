...
## 3. HITL 模型：一个机制 / 三种 form / 三个触发点

`HitlManager` 是单一机制（request → wait → resolve）。每个请求带显式 **`form`** 区分语义：

| form | 语义 | 应答 | 消费方 |
|------|------|------|--------|
| `approval` | 工具门控（放行/拒绝一次调用） | `approve` / `reject` | HumanConfirmationAuthorizer |
| `question` | 向人提问（取回文字答复） | `answer` / `reject` | request_human_input、submit_task_assessment(needs_user_input) |
...
host 据 `request.form` 决定动作与 UI：
- `approval` → `/hitl/{id}/approve`（可带 modify）或 `/reject`（渲染批准/拒绝）。
- `question` / `wait` → `/hitl/{id}/answer`（渲染答题输入框 / 普通输入框）或 `/reject`。
- 兼容入口（用户直接回话）：REJECT 词 → reject；否则 approval→approve、question/wait→answer。
...
缓存命中（冷路径）→ 直接返回缓存决定，不调 `wait()`。缓存未命中（热路径）→ `request(form="approval")` + `wait`，
...
**`question`/`wait` 的文字回复一律是答复**（`answer(text)`）——`"no"` 是一个否定**答复**，不是"拒绝问题"。
绝不可从答复文本用关键词推断 reject；question/wait 的显式拒答 / 中止只能走 `POST /hitl/{id}/reject` 或 interrupt。
...
- [ ] **决定与 message 正交**：question/wait 路径 accept→`message or 默认`、reject→`Human declined: {message}` 回灌。
- [ ] **关键词路由仅限 approval**；question/wait 文字回复一律为答复（`"no"` 是答复，非拒绝），显式拒答只走 reject 端点 / interrupt。
- [ ] resolve 幂等、`wait` 热窗口超时抛 `HitlPark`（hot→cold 降级）、未知 id 报错、host 按 form 路由应答。
