...
| 已终局 | 不再被 parked 挡住，由既有的重排规则自然放行 |

> **订正（2026-09-01，段 2 Task 9 审查发现）**：本节初稿要求「已终局 ⟹ **主动重排**」，并为此引入
> 一个豁免开关。**那是错的**，两个理由：
> ① **有害**。豁免会把父任务置成 `PENDING`，而子任务完成时的唤醒守卫是「父任务仍为 SUSPENDED」——
> 于是合法的唤醒被静默吞掉，父任务提前跑一次、拿到残缺的子输出，真正该跑的那次永远不来。
> ② **多余**。把豁免窄化成「无未终结子任务」后，它与既有的重排门完全等价，变成死代码。
>
> 真正需要修的是 **parked 集合的来源**：它此前取自投影，而投影的 reducer 没有新模型事件的分支，
> 于是新模型下未决的 park 折出来是空集 —— 任务在没人回答时就跑起来了。把 parked 改为取自
> registry 的未决集合，两个方向就都对了：未决被挡住，已终局自然放行。

**「宁可重排一次」仍然安全**，因为两条续跑路径本身都幂等（上面两条）。所以恢复期**不需要**记
「这次续跑到底跑没跑过」——那笔账要跨重启，就又需要一份持久状态，绕回 §3.1 想消除的东西。

#### 7.3.1.1 `UserTurn` 的答复必须由恢复期补注入

重排解决的是「任务要不要跑」，解决不了「人的答复有没有进去」。这两件事对 `ToolResult` 是同一件
（reconcile 会补上工具结果），对 `UserTurn` **不是**：注入只发生在应答入口的续跑路径上，恢复期
只重排、不注入。于是「答复已落盘、进程在注入前崩溃」的结果是——任务醒来重入，**答复从未进过对话**，
静默丢失。这正是本节要堵的窗口，却会从这里漏掉。

因此恢复期必须：**对每个已终局的 `UserTurn` 请求执行一次注入**。无条件执行是安全的，因为注入以
`hitl_id` 派生的 `MemoryEvent.id` 为幂等键——这个键存在的意义就是让它可以被重放。已经注入过则是
no-op；崩在注入之前，则这是唯一能把答复救回来的地方。

这条对**装填完备性**提出了第二个要求：装填集合必须覆盖**没有 `tool_call_id` 的请求**。`UserTurn`
的 park（`act` 的纯文本暂停与软打断）恰恰不带 `tool_call_id`，若以它为收录条件，整个 `UserTurn`
类都会被排除在兜底之外。决定缓存的三维键（§4.5）是安全边界、不能动；「本会话哪些请求已终局」
是另一件事，按 `hitl_id` 收录即可。
...
- [ ] **重启后一切皆冷**：等待槽全空。
- [ ] **恢复期的 parked 集合取自未决 HITL 的真实集合**（不是投影的有损副本）：挂在未决 HITL 上的保持 parked、不重排；已终局的不再被挡住，由既有重排规则放行。**不得**为「已终局」另加主动重排的豁免——它会把父任务置成 PENDING，吞掉子任务完成时的唤醒（§7.3.1 订正）。
- [ ] **恢复期对已终局的 `UserTurn` 请求补一次注入**（幂等键 = `hitl_id` 派生的 `MemoryEvent.id`）。只重排不注入 = 任务醒来但人的答复从未进过对话（§7.3.1.1）。装填集合因此必须收录**没有 `tool_call_id`** 的请求。
