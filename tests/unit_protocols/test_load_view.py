...
def _v2(kind: MemoryKind, layer: MemoryLayer, content: str, minute: int, *,
...
async def test_load_view_ascending_and_default_kinds() -> None:
    ...
    audit = await m.load_view(MemoryAddress(session_id="s1", task_id="t1", agent_id="a1"),
                              MemoryLayer.TASK, _ctx(), kinds=[MemoryKind.TOOL_AUDIT])
...
async def test_load_view_sees_both_vocabularies() -> None:
    ...
    await m.ingest(_v2(MemoryKind.CONVERSATION_TURN, MemoryLayer.TASK, "new-vocab", 1,
                       role="assistant"), _ctx())
...
async def test_recall_recent_wrapper_matches_v2_rows() -> None:
    ...
    await m.ingest(_v2(MemoryKind.CONVERSATION_TURN, MemoryLayer.TASK, "v-up", 1, role="user"), _ctx())
    await m.ingest(_v2(MemoryKind.CONVERSATION_TURN, MemoryLayer.TASK, "v-resp", 2,
                       role="assistant"), _ctx())
